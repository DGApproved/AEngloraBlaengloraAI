...
# ── FILE PATHS ───────────────────────────────────────────────
# BASE = "." resolves to the tenant node (e.g. fn/fn1/) as set by
# Java's ProcessBuilder working directory. All paths derive from there.
BASE="."
DGAPI="$BASE/dgapi"

# System files — hardware profile, hibernate state, session profile
...
# Knowledge files — long-term inspectable memory
...
# Intake pipeline — books, code, references the AI reads
...
# Session log — convodata.txt is directly in the tenant root (fn/fn1/)
SESSION_LOG="$BASE/convodata.txt"

# Virtual files — encyclopedia overlay, theory queue, persona, vocabulary
...
# Governance and tool state
SEPARATE_TXT="$BASE/separate.txt"             # governs resource and vocab sharing
RESOURCES_DIR="$DGAPI/resources"
TOOL_STATE="$DGAPI/tool_state.txt"
TOOL_STATE_SH="$DGAPI/tool_state_sh.txt"     # per-runtime fallback if separate.txt = "yes"
SHARED_TXT="$BASE/shared.txt"                # tool pipeline differential map
...
# ── [ACTION] TAG EMITTER ──────────────────────────────────────
# Emits [ACTION] to stdout so the Java Mode B avatar reflects
# what the AI is doing. Novel action words are recorded in vocab.

emit_action() {
    local action="$1"
    printf '[ACTION] %s\n' "$action"
    # Register novel actions in vocabulary
    local key; key="ACTION_$(printf '%s' "$action" | tr '[:lower:] ' '[:upper:]_')"
    vocab_append_new "$key" "$action" "sh:"
}

# ── FIND IMAGE GGUF ───────────────────────────────────────────
# Image GGUF must contain "(image)" in its filename.
# Text GGUF must NOT contain "(image)".
...
find_image_gguf() {
    ...
    for f in "$BASE"/*.gguf; do
        [ -f "$f" ] || continue
        case "$(basename "$f")" in
            *\(image\)*) IMAGE_GGUF_FILE="$f"; return 0 ;;
        esac
    done
    return 1
}

# Override find_gguf to exclude image models
find_gguf() {
    local f
    for f in "$BASE"/*.gguf; do
        [ -f "$f" ] || continue
        case "$(basename "$f")" in
            *\(image\)*) continue ;;  # skip image models
            *) GGUF_FILE="$f"; return 0 ;;
        esac
    done
    return 1
}
...
# ── TOOL STATE ────────────────────────────────────────────────
# tool_state.txt: plain key=value, NAME=enabled|disabled
# Shared unless separate.txt = "yes".

tool_state_file() {
    [ "$(read_separate)" = "yes" ] \
        && printf '%s' "$TOOL_STATE_SH" \
        || printf '%s' "$TOOL_STATE"
}
...
is_tool_enabled() {
    ...
    tsf=$(tool_state_file)
    [ -f "$tsf" ] || return 1
    grep -q "^${name}=enabled$" "$tsf" 2>/dev/null
}

set_tool_enabled() {
    [ "$ELEVATED" = true ] || return
    local name="$1" enabled="$2" tsf val
    tsf=$(tool_state_file)
    val=$([ "$enabled" = true ] && echo "enabled" || echo "disabled")
    if grep -q "^${name}=" "$tsf" 2>/dev/null; then
        sed -i "s|^${name}=.*|${name}=${val}|" "$tsf"
    else
        printf '%s=%s\n' "$name" "$val" >> "$tsf"
    fi
    journal_append "TOOL $([ "$enabled" = true ] && echo ENABLED || echo DISABLED): $name"
}

load_tool_state_count() {
    local tsf; tsf=$(tool_state_file)
    [ -f "$tsf" ] && grep -c "=enabled$" "$tsf" 2>/dev/null || echo 0
}

# ── TOOL MANIFEST SCANNER ────────────────────────────────────
# Scans /resources/*.manifest at startup and on hot-load.
# Each manifest is a plain key=value file:
...
scan_manifests() {
    ...
    MANIFEST_COUNT=0
    local mf name desc tags binary

    # Initialise shared.txt header
    if [ "$ELEVATED" = true ]; then
        {
            printf '# shared.txt — tool pipeline differential map\n'
            printf '# Format: OUTPUT_OF_TOOL->INPUT_TO_TOOL : FIELD_MAPPING\n'
            printf '# Written by resource scanner. AI may extend this file.\n'
        } > "$SHARED_TXT"
    fi

    for mf in "$RESOURCES_DIR"/*.manifest; do
        [ -f "$mf" ] || continue
        name=$(grep -m1 "^NAME="    "$mf" | cut -d= -f2-)
        desc=$(grep -m1 "^DESCRIPTION=" "$mf" | cut -d= -f2-)
        tags=$(grep -m1 "^TAGS="   "$mf" | cut -d= -f2-)
        binary=$(grep -m1 "^BINARY=" "$mf" | cut -d= -f2-)

        [ -z "$name" ] && continue

        # Write almanac entry so the reasoning engine can find this tool
        write_almanac_entry "TOOLS" "$name" \
            "${desc:-No description.}" "resources manifest"

        # Register tool in vocab
        local vkey; vkey="TOOL_$(printf '%s' "$name" | tr '[:lower:] ' '[:upper:]_')"
        vocab_append_new "$vkey" "${desc:0:60}" "sh:"

        # Auto-detect compatible pipelines for shared.txt
        if [ "$ELEVATED" = true ]; then
            local out_fmt in_fmt
            out_fmt=$(grep -m1 "^OUTPUT_FORMAT=" "$mf" | cut -d= -f2- | tr '[:upper:]' '[:lower:]')
            in_fmt=$( grep -m1 "^INPUT_FORMAT="  "$mf" | cut -d= -f2- | tr '[:upper:]' '[:lower:]')
            # Compare with every other manifest for field compatibility
            for mf2 in "$RESOURCES_DIR"/*.manifest; do
                [ -f "$mf2" ] && [ "$mf2" != "$mf" ] || continue
                local name2 in2
                name2=$(grep -m1 "^NAME="         "$mf2" | cut -d= -f2-)
                in2=$(  grep -m1 "^INPUT_FORMAT="  "$mf2" | cut -d= -f2- | tr '[:upper:]' '[:lower:]')
                # If a word from our output format appears in their input format — compatible
                for word in $out_fmt; do
                    [ "${#word}" -gt 3 ] || continue
                    if printf '%s' "$in2" | grep -qw "$word" 2>/dev/null; then
                        printf '%s>%s : OUTPUT=%s INPUT=%s\n' \
                            "$name" "$name2" "$out_fmt" "$in2" >> "$SHARED_TXT"
                        break
                    fi
                done
            done
    ...
    [ "$MANIFEST_COUNT" -gt 0 ] && \
        journal_append "RESOURCE SCAN: $MANIFEST_COUNT manifest(s) found"
}

# ── TOOL INTENT CLASSIFICATION ────────────────────────────────
# Determines if a query enables, disables, or invokes a tool.
# Uses GGUF if available and working; keyword fallback otherwise.
# Returns: sets TOOL_INTENT_ACTION (enable|disable|use|none)
#                TOOL_INTENT_NAME  (tool name or empty)

TOOL_INTENT_ACTION="none"
TOOL_INTENT_NAME=""
...
classify_tool_intent() {
    ...
    [ -d "$RESOURCES_DIR" ] || return
    local known_names=""
    for mf in "$RESOURCES_DIR"/*.manifest; do
        [ -f "$mf" ] || continue
        local n; n=$(grep -m1 "^NAME=" "$mf" | cut -d= -f2-)
        [ -n "$n" ] && known_names="$known_names $n"
    done
    [ -z "$known_names" ] && return

    # ── GGUF intent classification ────────────────────────────
    if [ -n "$GGUF_FILE" ] && [ -n "$LLAMA_BIN" ]; then
        local names_list prompt raw
        names_list=$(printf '%s' "$known_names" | xargs | tr ' ' ',')
        prompt="Available tools: $names_list\nUser query: \"$query\"\nDoes this query ask to enable, disable, or use one of the tools?\nReply with exactly one of:\nENABLE <tool_name>\nDISABLE <tool_name>\nNONE\nAnswer:"
        raw=$(query_gguf "$prompt" 20 | tr '[:lower:]' '[:upper:]' | xargs)
        case "$raw" in
            ENABLE*)
                local tname; tname=$(printf '%s' "$raw" | cut -d' ' -f2- | tr '[:upper:]' '[:lower:]' | xargs)
                for n in $known_names; do
                    printf '%s' "$tname" | grep -qi "$n" && TOOL_INTENT_ACTION="enable" \
                        && TOOL_INTENT_NAME="$n" && return
                done ;;
            DISABLE*)
                local tname; tname=$(printf '%s' "$raw" | cut -d' ' -f2- | tr '[:upper:]' '[:lower:]' | xargs)
                for n in $known_names; do
                    printf '%s' "$tname" | grep -qi "$n" && TOOL_INTENT_ACTION="disable" \
                        && TOOL_INTENT_NAME="$n" && return
                done ;;
        esac
        return
    ...
    for n in $known_names; do
        ...
        # Disable patterns
        for pat in "stop using " "disable " "deactivate " "turn off " "stop "; do
            printf '%s' "$ql" | grep -q "${pat}${nl}" && \
                TOOL_INTENT_ACTION="disable" && TOOL_INTENT_NAME="$n" && return
        done
        # Enable patterns
        for pat in "use " "look at " "enable " "activate " "run " "launch " "start using "; do
            printf '%s' "$ql" | grep -q "${pat}${nl}" && \
                TOOL_INTENT_ACTION="enable" && TOOL_INTENT_NAME="$n" && return
        done
    done
}

execute_tool() {
    # Usage: execute_tool MANIFEST_FILE QUERY_INPUT
    # Returns tool output on stdout
    [ "$ELEVATED" = true ] || { printf '[Tool execution blocked: restricted mode]\n'; return; }
    local mf="$1" input="$2"
    local name binary args_template
    name=$(    grep -m1 "^NAME="   "$mf" | cut -d= -f2-)
    binary=$(  grep -m1 "^BINARY=" "$mf" | cut -d= -f2-)
    args_template=$(grep -m1 "^ARGS=" "$mf" | cut -d= -f2-)

    local bin_path="$RESOURCES_DIR/$binary"
    [ -x "$bin_path" ] || { printf '[Tool %s: binary not found at %s]\n' "$name" "$bin_path"; return; }

    # Substitute {INPUT} placeholder
    local args; args=$(printf '%s' "${args_template:-{INPUT}}" | sed "s|{INPUT}|${input}|g")

    local out rc
    out=$(cd "$RESOURCES_DIR" && "$bin_path" $args 2>&1)
    rc=$?
    journal_append "TOOL EXEC: $name | exit:$rc | out:${#out}c"

    # Append last result to shared.txt for pipeline awareness
    [ "$ELEVATED" = true ] && \
        printf '# Last run: %s -> %s\n' "$name" "${out:0:80}" >> "$SHARED_TXT"

    printf '%s\n' "$out"
}

# Try enabled tools relevant to the current query
# Sets TOOL_OUTPUT if a tool ran
TOOL_OUTPUT=""
...
try_enabled_tools() {
    ...
    for mf in "$RESOURCES_DIR"/*.manifest; do
        ...
        local name tags
        name=$(grep -m1 "^NAME=" "$mf" | cut -d= -f2-)
        tags=$(grep -m1 "^TAGS=" "$mf" | cut -d= -f2- | tr '[:upper:]' '[:lower:]')
        is_tool_enabled "$name" || continue
        # Check if any tag word appears in the query
        for tag in $tags; do
            [ "${#tag}" -gt 3 ] || continue
            if printf '%s' "$ql" | grep -qw "$tag"; then
                emit_action "USING_TOOL"
                TOOL_OUTPUT=$(execute_tool "$mf" "$query")
                return
            fi
        done
    done
...
# ── IMAGE GGUF — intake image processing ─────────────────────
# If a .gguf with "(image)" in filename is present and llama-cli
# supports vision, images from intake/ are described and stored
# in the encyclopedia pipeline.
# IMAGE_EXT files are detected; llava-style CLI flags used.

IMAGE_EXT_LIST="png jpg jpeg webp bmp gif"

image_ext_supported() {
    local ext="${1##*.}"
    printf '%s' "$IMAGE_EXT_LIST" | grep -qw "${ext,,}"
}

process_intake_image() {
    local fp="$1"
    [ "$GGUF_WRITE_ALLOWED" = true ]   || return
    [ -n "$IMAGE_GGUF_FILE" ]          || return
    [ -n "$LLAMA_BIN" ]                || return

    local fname; fname=$(basename "$fp")
    status "STATE: READING | $fname (image)"

    # llama.cpp image description via --image flag (llava/moondream style)
    local description
    description=$("$LLAMA_BIN" \
        -m "$IMAGE_GGUF_FILE" \
        --image "$fp" \
        -p "Describe this image in 2-3 sentences." \
        -n 200 \
        --log-disable \
        --no-display-prompt \
        2>/dev/null | head -c 600)
    ...
    # Propose theory via religion.txt pipeline
    local uid; uid="img_$(printf '%s' "${fname%.*}" | tr '[:upper:] ' '[:lower:]_' | cut -c1-30)"
    religion_propose_theory "$uid" "TOPIC" \
        "TOPIC=$fname" \
        "PROPOSED_TEXT=${description:0:200}" \
        "SOURCE=$(basename "$IMAGE_GGUF_FILE")"

    religion_promote_ready 3
    chat "Image processed: $fname"
    journal_append "IMAGE GGUF: $fname | desc len:${#description}"

    # Move to processed
    local dest="$PROCESSED_DIR/$fname"
    [ -f "$dest" ] && dest="$PROCESSED_DIR/${fname%.*}_$(ts_unix).${fname##*.}"
    mv "$fp" "$dest"
}

process_all_intake_images() {
    [ -n "$IMAGE_GGUF_FILE" ] || return
    [ -n "$LLAMA_BIN" ]       || return
    local fp
    for dir in "$BOOKS_DIR" "$CODE_DIR" "$REFERENCE_DIR"; do
        ...
        for fp in "$dir"/*; do
            [ -f "$fp" ] || continue
            image_ext_supported "$fp" || continue
            GGUF_WRITE_ALLOWED=true
            process_intake_image "$fp"
            GGUF_WRITE_ALLOWED=false
        done
    done
}
...
# Ensure all structural directories exist (matches buildMatrixArchitecture in Java)
mkdir -p "$SYSTEM_DIR" "$DATAS_DIR" "$VIRTUAL_DIR" \
         "$BOOKS_DIR" "$CODE_DIR" "$REFERENCE_DIR" "$PROCESSED_DIR" \
         "$RESOURCES_DIR" "$DGAPI/experiments"
...
handle_query() {
    ...
    # ── TOOL INTENT CLASSIFICATION ───────────────────────────────
    classify_tool_intent "$query"
    local tool_output=""
    case "$TOOL_INTENT_ACTION" in
        enable)
            set_tool_enabled "$TOOL_INTENT_NAME" true
            chat "Tool enabled: $TOOL_INTENT_NAME"
            emit_action "TOOL_ENABLED"
            ;;
        disable)
            set_tool_enabled "$TOOL_INTENT_NAME" false
            chat "Tool disabled: $TOOL_INTENT_NAME"
            emit_action "TOOL_DISABLED"
            ;;
        *)
            # Check if any enabled tool is relevant to this query
            try_enabled_tools "$query"
    ...
    [ -n "$tool_output" ] && chat "Tool output: ${tool_output:0:300}"
    ...
    # Vocab evolution at prompt time
    vocab_set "LAST_QUERY_LEN"       "${#query}"
    vocab_set "SESSION_QUERY_COUNT"  "$QUERY_COUNT"
...
do_shutdown() {
    ...
    if [ -n "$GGUF_FILE" ] && [ -n "$LLAMA_BIN" ]; then
        GGUF_WRITE_ALLOWED=true
        gguf_shutdown_enrichment
        # Final resource scan enrichment pass
        [ "$MANIFEST_COUNT" -gt 0 ] && scan_manifests
        GGUF_WRITE_ALLOWED=false
    fi
    # ─────────────────────────────────────────────────────────────

    # Vocab evolution: record session summary
    vocab_set "LAST_SESSION_QUERIES" "$QUERY_COUNT"
...
# Image intake (requires image GGUF — gate opened below after detection)
if find_image_gguf && find_llama_bin; then
    GGUF_WRITE_ALLOWED=true
    process_all_intake_images
    GGUF_WRITE_ALLOWED=false
fi
...
scan_manifests
[ "$MANIFEST_COUNT" -gt 0 ] && {
    chat "Resources: $MANIFEST_COUNT tool(s) found in /resources."
    vocab_set "TOOL_COUNT" "$MANIFEST_COUNT" "sh:"
}
...
# Vocab evolution at startup
vocab_set "LAST_BOOT_INTAKE_COUNT" "$INTAKE_COUNT"
vocab_set "REASONING_TIER" "${CACHE_REASONING_TIER:-CPU_ONLY}"
