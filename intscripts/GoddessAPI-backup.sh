...
encyclopedia_write_entry() {
    # Usage: encyclopedia_write_entry TYPE KEY=value KEY=value ...
...
gguf_enrich_word() {
    ...
    local defn syns
    defn=$(query_gguf \
        "You are the Goddess Matrix, a fractal AI running on isolated, local hardware. Write a single-sentence definition for the concept of '$word' as it relates to our digital architecture.\nDefinition:" 80)
    syns=$(query_gguf \
...
SESSION_PROFILE="$DATAS/session_profile.txt"
JOURNAL="$DATAS/journal.txt"
DICTIONARY="$DATAS/dictionary.txt"
THESAURUS="$DATAS/thesaurus.txt"
ALMANAC="$DATAS/almanac.txt"
ENCYCLOPEDIA="$DATAS/encyclopedia.txt"   # GGUF overlay
RELIGION="$DATAS/religion.txt"           # theory queue
...
mkdir -p "$STORAGE" "$DATAS" "$BOOKS_DIR" "$CODE_DIR" "$REFERENCE_DIR" "$PROCESSED_DIR"
...
intake_watcher_inotify() {
    ...
    | while IFS=' ' read -r dir _events fname; do
        ...
        # ATOMIC LOCK: Wait for the OS to release the file before reading
        while lsof "$fp" &>/dev/null; do
            sleep 1
        done
